...
var mic = function mic(options) {
    ...
    if(libPath.length && libPath.substr(-1) !== ' ') {
        libPath += ' '
    }
    ...
    that.start = function start() {
          if(audioProcess === null) {
            if(isWindows){
              audioProcess = spawn(libPath + 'sox', ['-b', bitwidth, '--endian', endian,
                                   '-c', channels, '-r', rate, '-e', encoding,
                                   '-t', 'waveaudio', 'default', '-p'],
                                    audioProcessOptions)
            }
            else if(isMac){
              audioProcess = spawn(libPath + 'rec', ['-b', bitwidth, '--endian', endian,
                                    '-c', channels, '-r', rate, '-e', encoding,
                                    '-t', fileType, '-'], audioProcessOptions)
            }
            else {
              audioProcess = spawn(libPath + 'arecord', ['-t', fileType, '-c', channels, '-r', rate, '-f',
